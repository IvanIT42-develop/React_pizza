...
function PizzaCard({ prices, title, imageUrl, sizes, types, imageMap, id }: PizzaCardProps) {
  ...
  const imageSrc = imageMap?.[(imageUrl ?? '' ) as keyof images]  || '';

  // 2. ДИНАМИЧЕСКИЙ СЧЁТЧИК: считаем, сколько таких пицц именно этого размера сейчас лежит на сервере
  const pizzaCount =
    cartItems?.filter((item) => item.title === title && item.size === sizes[activeSize]).length ||
    0;
    ...
    <div className={classes.parent} data-id={id}>
      ...
      <div className={classes.typesandsizes}>
        <div className={classes.typesOfPizza}>
          {types.map((text: number, index: number) => (
            <li
              key={`${id}-type-${index}`}
